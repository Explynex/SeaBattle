...
void aboutgame()
{
    ...
    std::cout << " • Как расставлять корабли? - Начать новую игру и выбрать: «Расставить вручную».";
    GotoXY((width - 70) / 2+3, (height - 5) / 2 +2);
    std::cout << "После нажатия кнопки выбора длины нужно кликнуть ЛКМ в нужном месте на поле для установки блока корабля.";
    GotoXY((width - 70) / 2, (height - 5) / 2 +3);
    std::cout << " • Как сохранить расстановку? - Расставить вручную , сгенерировать автоматически или загрузить из файла.";
    GotoXY((width - 70) / 2+3, (height - 5) / 2 +4);
    std::cout << "Нажать кнопку «yes» для сохранения или «no» чтобы пропустить момент сохранения.";
    GotoXY((width - 70) / 2, (height - 5) / 2 +5);
    ...
    std::cout << " • Для возврата основное в меню из любого места требуется нажать «Esc».";
    setColor(LIGHTRED, BLACK);
    GotoXY((width - 20) / 2 , (height - 5) / 2 + 9);
